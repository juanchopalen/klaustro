    ...
    <div>
      ...
      <section class="section section-xl bg-gray-lighter">
        <div class="container container-bigger" :class="loading ? 'loading' : ''">
          <template v-if="user">
            <router-link to="/blog/create-post"><h4><span class="icon novi-icon icon-md icon-primary mdi mdi-plus"></span> {{ translate(blog, 'add_post') }}</h4></router-link>
          </template>
          <div class="row row-50 row-md-90 justify-content-md-center justify-content-xl-between">
            ...
            <transition name="bounce" enter-active-class="bounceInLeft" leave-active-class="bounceOutRight">
              <p v-if="posts.length == 0 && !loading" class="breadcrumbs-custom-subtitle">{{ translate(blog, 'not_found') }}</p>
            </transition>
                    ...
                    <div class="post-blog-caption">
                      <div class="post-blog-caption-header">
                        ...
                        <ul class="post-blog-meta">
                          <li><span>{{ translate(blog, 'author') }}</span>&nbsp;<a href="#">{{ post.author }}</a></li>
                        </ul>
                      </div>
            ...
            <div class="col-md-10 col-lg-4 blog-aside">
              <div class="blog-aside-item">
                <h6>{{ translate(blog, 'search') }}</h6>
                <form class="rd-search rd-search-modern">
                  <div class="form-wrap">
                    <input v-model="target" @keyup.enter="getAllPosts" class="rd-search-input form-input" id="rd-search-input" type="text" name="s" autocomplete="off" :placeholder="translate(blog, 'search')">
                  </div>
              ...
              <div class="blog-aside-item">
                ...
                <ul class="list-marked list-marked-secondary">
                  <li><a href="##" @click="getAllPosts">{{ translate(blog, 'all_posts') }}</a></li>
              ...
              <div class="blog-aside-item">
                <h6>{{ translate(blog, 'about') }}</h6>
                <p>{{ translate(blog, 'about_content') }}</p><router-link class="button button-xs button-default-outline button-nina" to="/">{{ translate(blog, 'button') }}</router-link>
              </div>
              <div class="blog-aside-item">
                <h6>{{ translate(blog, 'categories') }}</h6>
                <ul class="list-marked list-marked-secondary">
                  <li><a href="##" @click="getAllPosts">{{ translate(blog, 'all_categories') }}</a></li>
